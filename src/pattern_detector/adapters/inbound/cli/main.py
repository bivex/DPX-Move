"""Typer and Rich CLI interface for DPX-Move."""
...
app = typer.Typer(
    name="dpx-move",
    help="📦 Enterprise Move Language Static Analyzer: Resource-Oriented Programming, Sui Objects, Aptos Storage, Abilities (key/store/copy/drop), Hot Potato, Move Prover Specs & GoF 23.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Move source file (.move) or directory containing a Move package (Move.toml).",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'resource_linear_type_safety', 'hot_potato_transient_receipt').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e build -e .move).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Move smart contract package for linear resources, ability composition, Sui objects, Move Prover specs, GoF patterns, and security hazards."""
...
@app.command()
def rules() -> None:
    """List all available Move detection rules and quality checks."""
    table = Table(
        title="📦 DPX-Move: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'resource_linear_type_safety', 'hot_potato_transient_receipt').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Move pattern."""
    ...
    panel = Panel.fit(
        ...
        f"[bold white]Move Target:[/bold white] {pdef.move_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Follow standard Move language best practices.'}",
        title=f"🔍 Move Architecture Encyclopedia: {pdef.name}",
        border_style="cyan",
    )
    console.print(panel)
