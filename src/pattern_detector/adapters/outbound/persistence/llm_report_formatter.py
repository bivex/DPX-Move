"""Token-Efficient LLM / AI Prompt Context Formatter for Move."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="move">',
        ...
        patterns = [d for d in report.detections if d.pattern_category not in (PatternCategory.PRINCIPLE, PatternCategory.MOVE_SECURITY_HAZARDS)]
        hazards = [d for d in report.detections if d.pattern_category in (PatternCategory.PRINCIPLE, PatternCategory.MOVE_SECURITY_HAZARDS)]

        if patterns:
            lines.append("    <move_resource_patterns_and_abilities>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <move_security_hazards>")
            for v in hazards:
