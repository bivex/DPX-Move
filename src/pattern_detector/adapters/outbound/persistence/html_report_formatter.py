"""Interactive HTML Observability HUD Formatter for Move Language."""
...
CATEGORY_COLORS: dict[PatternCategory, dict[str, str]] = {
    PatternCategory.MOVE_IDIOMATIC_RESOURCES: {
        "text": "#00e5ff",
        "bg": "#00e5ff22",
        "border": "#00e5ff",
        "accent": "#4ff0ff",
        "name": "Linear Resources & Abilities",
    },
    PatternCategory.OBJECT_TABLE_MODELS: {
        "text": "#2a82e4",
        "bg": "#2a82e422",
        "border": "#2a82e4",
        "accent": "#5ca5fa",
        "name": "Sui Object & Aptos Storage",
    },
    PatternCategory.FORMAL_SPEC_PROVER: {
        "text": "#50fa7b",
        "bg": "#50fa7b22",
        "border": "#50fa7b",
        "accent": "#73fa96",
        "name": "Move Prover & Formal Specs",
    },
    ...
    PatternCategory.MOVE_SECURITY_HAZARDS: {
        "text": "#ff5555",
        "bg": "#ff555522",
        "border": "#ff5555",
        "accent": "#ff6e6e",
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders standalone, responsive HTML dashboard for Move DetectionReport with AI Prompt Generator."""
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            finding_prompt = (
                f"Refactor and optimize the following Move smart contract architectural finding:\n"
                ...
                f"Please provide an idiomatic Move 2024 / Aptos / Sui smart contract implementation adhering to linear resource safety, strict ability composition (key, store, copy, drop), capability access control, and Move Prover formal specifications."
            )
...
<head>
    ...
    <title>Pattern Scanner Report - DPX-Move Architecture HUD - {html.escape(report.project_path or "Codebase")}</title>
    <style>
        :root {{
            --bg: #090e17;
            --card-bg: #111a28;
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, monospace, sans-serif; }}
        ...
        .btn-ai-context {{
            background: linear-gradient(135deg, #00e5ff 0%, #2a82e4 100%);
            color: #090e17;
            border: none;
            padding: 10px 18px;
            border-radius: 8px;
        ...
        .btn-all {{ background: #1a2538; color: #f0f6fc; border-color: #2e3e5c; }}
        ...
        .search-bar:focus {{ border-color: var(--move-cyan); }}
        ...
        .pattern-card:hover {{ border-color: #00e5ff88; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }}
        .card-header {{ background: #162235; padding: 12px 18px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid var(--border); }}
        ...
        code {{ background: #090e17; padding: 3px 6px; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; color: #00e5ff; border: 1px solid #1e2c42; }}
        ...
        .location-tag {{ color: #00e5ff; font-size: 11px; margin-left: 6px; }}
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>📦 DPX-Move Architecture & Resource Safety Observability HUD</h1>
                <div class="subtitle">Linear Resources, Abilities (key/store/copy/drop), Sui Objects, Aptos Storage, Hot Potato & GoF 23 • Target: <code>{html.escape(report.project_path or "Target Repository")}</code></div>
            </div>
        ...
        <input type="text" id="searchInput" class="search-bar" placeholder="🔎 Instant search by pattern name, category, target struct/function, or ability (e.g. key, hot_potato, witness, capability, sui, prover)...">
    ...
    <div id="promptModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title">🤖 AI Smart Contract Architect Context Prompt (Move Language)</div>
                <button class="modal-close" onclick="closePromptModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 13px; color: #94a3b8; margin-bottom: 12px;">Structured XML/Markdown context for LLMs (Claude, GPT-4, Gemini) detailing linear types, ability composition, Sui objects, Aptos storage, Move Prover specs, and security hazards.</p>
    ...
    <script>
        ...
        filterBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                btn.style.outline = '2px solid #00e5ff';
                filterCards();
            }});
        }});
    </script>
