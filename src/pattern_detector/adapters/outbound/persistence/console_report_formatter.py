"""Console Rich Formatter for Move pattern detection findings."""
...
class ConsoleReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        # Header Banner
        header = Panel.fit(
            f"[bold white]📦 DPX-Move: Linear Resources, Sui Objects, Abilities, Hot Potato & GoF 23 (Move 2024 / Aptos / Sui)[/bold white]\n"
        ...
        table.add_column("Detections", justify="right", style="bold cyan")
        ...
        console.print(table)
        console.print("\n[bold]📋 Identified Move Smart Contract Patterns & Security Signals:[/bold]\n")
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            if det.primary_location:
                tree.add(f"📍 [dim]Location:[/dim] [cyan]{det.primary_location}[/cyan]")
            ...
            if det.evidences:
                ...
                for ev in det.evidences:
                    ...
                    loc_str = f" → [cyan]{ev.location}[/cyan]" if ev.location else ""
