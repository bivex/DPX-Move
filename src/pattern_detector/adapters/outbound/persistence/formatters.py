"""JSON, Markdown, SARIF formatters & persistence repositories for Move."""
...
class MarkdownReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        lines = [
            f"# 📦 DPX-Move: Move Smart Contract Architectural Pattern Report",
            f"",
        ...
        lines.extend([
            f"",
            f"## 🔍 Detailed Pattern Instances & Security Findings",
            f"",
        ])
...
class SarifReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for det in report.detections:
            ...
            level = "error" if det.pattern_category.value in ("move_security_hazards", "principle") else "note"
            ...
            "runs": [
                {
                    "tool": {
                        "driver": {
                            "name": "DPX-Move",
                            "informationUri": "https://github.com/bivex/DPX-Move",
                            "version": "0.1.0",
                        }
                    },
                    "results": results,
