"""Inbound driving ports for DPX-Move."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Move scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".move"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Move source file."""
