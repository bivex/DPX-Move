"""Outbound driven ports for DPX-Move."""

from __future__ import annotations

from typing import Protocol
from pattern_detector.domain.code_model import CodeModel, MoveFile
from pattern_detector.domain.detection import DetectionReport


class SourceProviderPort(Protocol):
    """Port for discovering and reading Move source files (.move)."""
...
class ParserPort(Protocol):
    """Port for parsing Move source text into CodeModel."""

    def parse_file(self, file_path: str, content: str) -> MoveFile:
        """Parse a single Move file into MoveFile model."""
        ...

    def parse_codebase(self, files: list[tuple[str, str]], target_path: str = "") -> CodeModel:
        """Parse multiple Move files into an aggregated CodeModel."""
        ...
